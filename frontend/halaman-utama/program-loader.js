...
function renderProgram(container, program) {
    ...
    <section class="hero detail-program-hero" style="padding:220px 0 120px">${detailSliderHtml(sliderImages, program.title, 'detail-hero-slider', program.image_alt || program.title)}<div class="hero-overlay" style="background:rgba(10,38,71,.8)"></div><div class="container hero-container" style="text-align:center"><div class="hero-content" style="max-width:100%;margin:0 auto"><h5 style="color:#64b5f6;letter-spacing:2px">${escapeHtml(program.category || 'LAYANAN DDU')}</h5><h1 style="color:white;margin-top:10px">${escapeHtml(program.hero_title || program.title)}</h1><p style="color:#cbd5e0">${escapeHtml(program.hero_subtitle)}</p></div></div></section>
